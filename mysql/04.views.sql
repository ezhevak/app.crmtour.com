...
AS SELECT
   ...
   `d`.`DealSumFact` AS `DealSum`,round((`d`.`DealSumFact` - `d`.`OperatorInvoceSum`),2) AS `DealSumPremia`,
   `d`.`DealSumEquivalent` AS `DealSumEquivalent`,
   `d`.`AgentClient` AS `AgentClient`,
   `d`.`UserId` AS `UserId`,
   `u`.`ManagerName` AS `ManagerName`,
   `u`.`Commission` AS `Commission`,round(((`d`.`DealSumFact` - `d`.`OperatorInvoceSum`) * (`u`.`Commission` / 100)),2) AS `ManagerPremia`,
   `d`.`DirectionId` AS `DirectionId`,
   `dir`.`DirectionName` AS `DirectionName`,
   `d`.`OperatorId` AS `OperatorId`,
   `op`.`Name` AS `OperatorName`,
