...
require $GLOBALS['RootDir'].'vendor/PHPExcel/Classes/PHPExcel.php';
require_once $GLOBALS['RootDir'].'vendor/phpmailer/PHPMailerAutoload.php';

function writeLog($text) {
    //return false;
    $fp = fopen ($GLOBALS['RootDir'].'log/srvtask_export/'.date("Ymd").'.txt', "a");
...
$address = $argv[3];
$uniqId=$AccId."-".$model;
writeLog($uniqId." start");

//$model = new Model_Leads();
//$select=$model->exportSQL()." where AccId='$AccId'";
if ($model == "Leads") {
...
writeLog($uniqId." File saved on server");
...
//$mail->addAddress('info@crmtour.com');
	...
	if(!$mail->send()) {
	    ...
	    writeLog($uniqId.' Mailer Error: ' . $mail->ErrorInfo);
	} else {
	    //echo 'Message has been sent';
	    writeLog($uniqId." Mail sended");
	}
